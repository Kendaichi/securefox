  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-sf-surface/90 backdrop-blur-md border-b border-sf-green/15'
          : 'bg-transparent'
      }`}
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 flex items-center justify-between h-[72px]">
        <Link to="/" className="flex items-center gap-3 shrink-0">
          <img src={logo} alt="SecureFox Logo" className="h-10 w-auto" />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className={`font-rajdhani font-600 text-sm uppercase tracking-wider transition-colors ${
